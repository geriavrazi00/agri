...
function selectCategories(category, categories) {
	...
	categoryDivElement = document.getElementById("category-" + category.id + "-div");
	...
	if (categoryElement.style.display === "none") {
		...
		for (var i = 0; i < category.option_number; i++) {
			...
			document.getElementById("business-0-" + i + "-" + category.id) != null 
			? document.getElementById("business-0-" + i + "-" + category.id).value = 1 
			: document.getElementById("business-0-" + i + "-" + category.id);

			document.getElementById("business-1-" + i + "-" + category.id) != null  
			? document.getElementById("business-1-" + i + "-" + category.id).value = ""
			...
			for (var j = 2; j < businessVariableNumber; j++) {
				document.getElementById("business-" + j + "-" + i + "-" + category.id)  != null  
				? document.getElementById("business-" + j + "-" + i + "-" + category.id).value = ""
				: document.getElementById("business-" + j + "-" + i + "-" + category.id);
			}
		}
...
function loanInterestRateValidation(input, min, max, requiredMessage, lessThanMinMessage, moreThanMaxMessage) {
	if (input.value == '') {
        input.setCustomValidity(requiredMessage);
...
function businessDataValidation(element, cultureNumber, optionNumber, categoryId) {
	document.getElementById("business-2-" + optionNumber + "-" + categoryId).setCustomValidity('');
	
	var isAnySelected = false;
	
	for(var i = 0; i < cultureNumber; i++) {
		if(document.getElementById("business-" + (i+2) + "-" + optionNumber + "-" + categoryId).value != '') {
			isAnySelected = true;
			break;
		}
	}
	...
	if(!isAnySelected) {
		document.getElementById("business-2-" + optionNumber + "-" + categoryId).setAttribute("required", "");
	} else {
		document.getElementById("business-2-" + optionNumber + "-" + categoryId).removeAttribute("required");
	}
}
...
function manageSelectedCategories(category, enable) {
	...
	for (var i = 0; i < category.option_number; i++) {
		for (var j = 0; j < businessVariableNumber; j++) {
			if(enable) {
				document.getElementById("business-" + j + "-" + i + "-" + category.id).removeAttribute("disabled");
			} else {
				document.getElementById("business-" + j + "-" + i + "-" + category.id).setAttribute("disabled", "");
			}
		}
	}
}
